extends MenuButton

var effects:Array = []
var current_sel:int

func on_pressed(i):
	Rhythia.select_miss_effect(effects[i])

func on_effect_selected(selected_effect:NoteEffect):
	text = selected_effect.name
	get_popup().set_item_checked(current_sel,false)
	for i in range(effects.size()):
		var effect:NoteEffect = effects[i]
		if effect == selected_effect:
			current_sel = i
			
			get_popup().set_item_checked(i,true)

func _ready():
	var found:Array = Rhythia.registry_effect.get_items()
	for i in range(found.size()):
		var effect:NoteEffect = found[i]
		get_popup().add_check_item(effect.name,i)
		effects.append(effect)
		if effect == Rhythia.selected_miss_effect:
			current_sel = i
			get_popup().set_item_checked(i,true)
	Rhythia.connect("selected_miss_effect_changed",self,"on_effect_selected")
	get_popup().connect("id_pressed",self,"on_pressed")
	on_effect_selected(Rhythia.selected_miss_effect)
