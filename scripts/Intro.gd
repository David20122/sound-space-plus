...
func _process(delta):
	$Sprite3D.rotation_degrees.z += 32 * delta
	$Sprite3D2.rotation_degrees.z += 16 * delta
	$Sprite3D3.rotation_degrees.z += 8 * delta
	
	if running and not fading:
		$ColorRect.modulate.a += (0 - $ColorRect.modulate.a) * 0.25 * delta
	if panning:
		$Camera2.translation.y += (12 - $Camera2.translation.y) * 0.5 * delta
		$Camera2.rotation_degrees.y += (180 - $Camera2.rotation_degrees.y) * 1 * delta
	if fading:
