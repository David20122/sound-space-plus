	...
	if !ProjectSettings.get_setting("application/networking/enabled"):
		...
		if !yield(self,"_connection_test"):
			Globals.notify(
				...
				"Online maps will not be loaded - failed to connect to map database.\n\nMake sure your system clock is Synchronized!",
				"No Connection"
			)
...
func check_latest_version():
	if !(OS.has_feature("Windows") or OS.has_feature("X11")) or !ProjectSettings.get_setting("application/networking/enabled"): # or OS.has_feature("OSX")
...
func attempt_update():
	...
	# if OS.has_feature("OSX"): asset_name = "macos.zip"
	# ^ Include when OSX tests go well - helm
