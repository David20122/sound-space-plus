...
func on_pressed(i):
	SSP.select_miss_effect(effects[i])
...
func _ready():
	...
	for i in range(found.size()):
		...
		if effect == SSP.selected_miss_effect:
			current_sel = i
			get_popup().set_item_checked(i,true)
	SSP.connect("selected_miss_effect_changed",self,"on_effect_selected")
	get_popup().connect("id_pressed",self,"on_pressed")
	on_effect_selected(SSP.selected_miss_effect)
