...
func update_search_text(txt:String):
	search_text = txt
	emit_signal("search_updated",txt,difficulty_filter)

func update_search_dfil(dfil:Array):
	difficulty_filter = dfil
	emit_signal("search_updated",search_text,dfil)
...
func _ready():
	...
	for i in range(found.size()):
		...
		btn.song = map
		if map.warning != "":
			if map.warning.begins_with("!!! "): btn.get_node("Name").modulate = Color(1,0.4,0.4)
